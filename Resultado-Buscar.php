...
$query = "SELECT * FROM busca WHERE 1=1"; // Inicia com uma condição verdadeira
...
if (!empty($bairro)) {
    $query .= " AND bairro = '$bairro'"; // Adiciona a condição do bairro se estiver preenchida
}
...
<!-- </li><li class="u-nav-item"><a class="u-button-style u-nav-link u-text-active-custom-color-1 u-text-hover-custom-color-1" href="Buscar.html" style="padding: 10px 20px;">Buscar Ecoponto</a> -->
    ...
    echo "<h2>Ecopontos encontrados:</h2>";
    ...
    while ($dados = mysqli_fetch_array($selecionar)) {
      echo "<tr>
              <td data-label='Rua'>" . $dados['rua'] . "</td>
              <td data-label='Cidade'>" . $dados['cidade'] . "</td>
              <td data-label='Bairro'>" . $dados['bairro'] . "</td>
              <td data-label='CEP'>" . $dados['cep'] . "</td>
            </tr>";
  }
...
?>
<a class="u-button-style u-nav-link u-text-active-custom-color-1 u-text-hover-custom-color-1" href="Buscar.html" style="padding: 10px 20px;">Voltar</a>
          </div>
        </div>
      </div>
    </section>
